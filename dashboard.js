...
function renderAssetHealthPanel(assets) {
  ...
  if (assets.length > 600) {
    const more = document.createElement('div'); more.className = 'muted small'; more.style.gridColumn = '1/-1'; more.style.marginTop = '6px';
    more.textContent = `Showing first 600 of ${assets.length} assets — apply filters to narrow.`;
    el.appendChild(more);
  }

  // Summary numbers
  document.getElementById('apTotalAssets').textContent = assets.length.toLocaleString();
  document.getElementById('apNoComm').textContent = assets.filter(a=>a.noCommunication).length.toLocaleString();
  document.getElementById('apNoImage').textContent = assets.filter(a=>a.noImage).length.toLocaleString();
  document.getElementById('apNoDoor').textContent = assets.filter(a=>a.noDoor).length.toLocaleString();
  document.getElementById('apOEM').textContent = assets.filter(a=>String(a.cabinetType).toUpperCase()==='OEM').length.toLocaleString();
  document.getElementById('apRetrofit').textContent = assets.filter(a=>String(a.cabinetType).toUpperCase()==='RETROFIT').length.toLocaleString();
}
